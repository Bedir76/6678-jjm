...
def format_person(p, title):
    ...
    if p.get('GSM') and p.get('GSM') not in ["YOK", "", None]:
        res += f"📱 **GSM:** `{p.get('GSM')}`\n"
    ...
    # --- MEMLEKET DÜZELTME ---
    mem_il = p.get('MEMLEKETIL', p.get('NUFUSIL', ''))
    mem_ilce = p.get('MEMLEKETILCE', p.get('NUFUSILCE', ''))
    memleket = f"{mem_il} / {mem_ilce}".strip(" /")
    if not memleket: memleket = p.get('MEMLEKET', 'Bilinmiyor')
    res += f"🗾 **MEMLEKET:** `{memleket}`\n"
    
    # --- YAŞADIĞI YER DÜZELTME ---
    adr_il = p.get('ADRESIL', p.get('IL', ''))
    adr_ilce = p.get('ADRESILCE', p.get('ILCE', ''))
    yer = f"{adr_il} / {adr_ilce}".strip(" /")
    if not yer or yer == "": 
...
async def ad_soyad_sorgula(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if matches:
            # Not: API'den gelen tablo sırasına göre indexleri (k[0], k[1] vb.) kontrol et.
            # Senin JSON örneğine göre ADRES bilgisini de içeren bir map yapıyoruz.
            for i in range(0, min(len(matches), 18), 6):
                ...
                p = {
                    ...
                    "ANNEADI": k[3], 
                    "GSM": k[4], 
                    "MEMLEKETIL": k[5], # 6. sütun genelde memleket veya il olur.
                    "ADRESIL": k[5] # Eğer il verisi buradaysa Yaşadığı Yer'e de yazsın.
                }

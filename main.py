...
# --- AYARLAR ---
# ÖNEMLİ: Bu tokenı güvenlik nedeniyle sadece sen bilmelisin!
...
def format_person(p, title):
    ...
    # --- MEMLEKET (MEMLEKETIL + MEMLEKETILCE) ---
    mem_il = p.get('MEMLEKETIL', '')
    mem_ilce = p.get('MEMLEKETILCE', '')
    memleket = f"{mem_il} / {mem_ilce}".strip(" /")
    res += f"🗾 **MEMLEKET:** `{memleket if memleket else 'Bilinmiyor'}`\n"
    
    # --- YAŞADIĞI YER (ADRESIL + ADRESILCE) ---
    adr_il = p.get('ADRESIL', '')
    adr_ilce = p.get('ADRESILCE', '')
    yer_birlesik = f"{adr_il} / {adr_ilce}".strip(" /")
    
    if not yer_birlesik: # Eğer API'den gelen veriler boşsa yedek anahtarlara bak
        yer_birlesik = p.get('ADRES', p.get('GUNCELADRES', 'Bilinmiyor'))
        
    res += f"📍 **YAŞADIĞI YER:** `{yer_birlesik}`\n"
    ...
    try:
        ...
        if matches:
            for i in range(0, min(len(matches), 18), 6):
                k = matches[i:i+6]
                # Buradaki p yapısı format_person'a uyumlu hale getirildi
                p = {"TC": k[0], "AD": k[1], "DOGUMTARIHI": k[2], "ANNEADI": k[3], "GSM": k[4], "MEMLEKETIL": k[5]}
...
if __name__ == '__main__':
    ...
    print("Bot yeni format ile başlatılıyor...")
    app.run_polling()
